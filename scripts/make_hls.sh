...
if [ $# -ne 2 ]; then
  echo "Usage: $0 <input.mp4> <output_dir>"; exit 1
fi
...
if [ ! -f "$in" ]; then
  echo "⏭️  Skip: input not found → $in"
  exit 0
fi

mkdir -p "$outdir"

# 1080p/720p/480p ladder, stereo AAC, 4s segments, independent segments
...
echo "✓ HLS written → $outdir/master.m3u8"
